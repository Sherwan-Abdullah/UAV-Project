...
################################################################################################
# plot the signal power and quality on seperate maps in the same window #
# state the last signal power and quality as a table in the same window #
# fill the value' table with the same range defined for the map plot #
################################################################################################

last_modified = os.path.getmtime('lte_data.txt')

while True:
    current_modified = os.path.getmtime('lte_data.txt')
    if current_modified != last_modified:
        ...
        try:
            ...
            # Create Figure subplots: one for each map
            ...
            # Add all maps to the Figure subplots
            m1.add_to(fig1)
            m2.add_to(fig2)
            m3.add_to(fig4)
            m4.add_to(fig5)
